...
- **Foreign registries**: amaru can also install from non-amaru-native repos that follow the same flat shape (e.g. `vercel-labs/agent-skills`, `jeremylongshore/claude-code-plugins-plus-skills`) — those repos have no `amaru_registry.json`, no skillsets, and no per-item version tags, so installs use the `latest` constraint and content-hash drift detection.
